...
static bool valid_nick(const char *nick)
{
    ...
    regex_t r; regcomp(&r, rx, REG_EXTENDED);
...
static void handle_msg(client_t *c, const char *line)
{
    if (strncmp(line, "MSG ", 4) == 0) {
        ...
        if (!valid_nick(candidate)) {
            send(c->fd, "ERROR bad nick\n", strlen("ERROR bad nick\n"), 0);
            return;
        }
        for (client_t *p = clients; p; p = p->next)
            if (p != c && p->registered && strcmp(p->nick, candidate) == 0) {
                send(c->fd, "ERROR nick in use\n", strlen("ERROR nick in use\n"), 0);
                return;
            }
...
static int create_listener(const char *host, const char *port)
{
    ...
    int rc = getaddrinfo(*host ? host : NULL, port, &hints, &ai);
    if (rc != 0) fatal(gai_strerror(rc));
    ...
    for (p = ai; p; p = p->ai_next) {
        ...
        if (bind(s, p->ai_addr, p->ai_addrlen) == 0 && listen(s, SOMAXCONN) == 0) break;
        close(s); s = -1;
    }
    freeaddrinfo(ai);
    if (s < 0) fatal("bind/listen");
    return s;
}
...
int main(int argc, char *argv[])
{
    ...
    char *arg = strdup(argv[1]), *colon = strchr(arg, ':');
    if (!colon) { fprintf(stderr, "IP:PORT required\n"); return EXIT_FAILURE; }
    *colon = '\0'; char *host = arg; char *port = colon + 1;

    int lsock = create_listener(host, port);
    printf("Listening on %s:%s …\n", *host ? host : "0.0.0.0", port); fflush(stdout);

    fd_set master, rfds; FD_ZERO(&master); FD_SET(lsock, &master); int maxfd = lsock;

    for (;;) {
        rfds = master;
        if (select(maxfd + 1, &rfds, NULL, NULL, NULL) < 0) fatal("select");
        ...
        if (FD_ISSET(lsock, &rfds)) {
            ...
            client_t *c = calloc(1, sizeof *c); c->fd = fd; c->next = clients; clients = c;
            FD_SET(fd, &master); if (fd > maxfd) maxfd = fd;
            send(fd, "HELLO " PROTO_VERSION "\n", 7 + strlen(PROTO_VERSION), 0);
        }

        for (client_t *c = clients, *next; c; c = next) {
            next = c->next; if (!FD_ISSET(c->fd, &rfds)) continue;
            char line[MAX_LINE + 1];
            while (getline_nb(c, line, sizeof line)) {
                if (!c->registered && strncmp(line, "NICK ", 5) != 0) {
                    send(c->fd, "ERROR register first\n", strlen("ERROR register first\n"), 0);
                    continue;
                }
                handle_msg(c, line);
            }
            if (errno == 0 && c->buflen == 0) { FD_CLR(c->fd, &master); remove_client(c); }
        }
    }
    return EXIT_SUCCESS;
}
